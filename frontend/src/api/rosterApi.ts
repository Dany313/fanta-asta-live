...
export const getRoster = async (team_id: number): Promise<Roster[]> => {
    const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}`}/api/rosters/team/${team_id}`);
...
export const getRosterByLeague = async (league_id: number): Promise<Roster[]> => {
    const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}`}/api/rosters/league/${league_id}`);
    if (!response.ok) throw new Error('Errore nel recupero dei giocatori');
    return response.json();
};
